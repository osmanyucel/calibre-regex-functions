...
def create_reference(note_num):
    return "<a href=\"notes.htm#n{note_num}\" id=\"n{note_num}d\"><sup>[{note_num}]</sup></a>".format(note_num=note_num)


def create_footnote(file_name, note_num, content):
    return "<p><a href=\"{file_name}#n{note_num}d\" id=\"n{note_num}\">{note_num}</a> {content}</p>".format(
...
<body>
    <h2>Notlar</h2>    
        ...
        """.format(title)
    current_container().add_file("notes.htm", html.encode("UTF-16"), media_type="text/html")
...
def replace(match, number, file_name, metadata, dictionaries, data, functions, *args, **kwargs):
    ...
    append_note(create_footnote(file_name, note_num, match.group()))
